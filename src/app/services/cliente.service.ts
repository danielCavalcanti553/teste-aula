import { Injectable } from '@angular/core';
import { AngularFirestore } from '@angular/fire/firestore';
import { from, Observable } from 'rxjs';
import { Cliente } from '../model/cliente';
@Injectable()
/*
@Injectable({
  providedIn: 'root',
})*/
export class ClienteService {

    cliente : Cliente = new Cliente();

    constructor(private firestore: AngularFirestore) {
    }

    listaDeClientes() : Observable<any>{

        return from(new Observable(observe => {

            this.firestore.collection('cliente').snapshotChanges().subscribe(response => {
                let lista: Cliente[] = [];
                response.map(obj => {
                    let cliente: Cliente = new Cliente();
                    cliente.id = obj.payload.doc.id;
                    cliente.setData(obj.payload.doc.data());
                    lista.push(cliente);
                })
                observe.next(lista);
            }, (err) => {
                observe.error(err);
            });
            
        }));
        
    }


    buscaClientesPorId(id : string) : Observable<any>{

        return from(new Observable(observe => {

            this.firestore.collection('cliente').doc(id).snapshotChanges().subscribe(response => {
                
                let cliente: Cliente = new Cliente();
                cliente.id = response.payload.id;
                cliente.setData(response.payload.data());
                observe.next(cliente);
                
            }, (err) => {
                observe.error(err);
            });
            
        }));
        
    }

    cadastrar(obj : any) : Observable<any>{
        return from(new Observable(observe => {

            this.firestore.collection('cliente').add(obj).then(response=>{
                observe.next("Cadastrado com sucesso");
            }, (err) => {
                observe.error("Erro ao cadastrar");
            });

        }));
    }

    atualizar(obj : any) : Observable<any>{
        return from(new Observable(observe => {

            this.firestore.collection('cliente').doc(obj.id).set(obj).then(response=>{
                observe.next("Atualizado com sucesso");
            }, (err) => {
                observe.error("Erro ao cadastrar");
            });

        }));
    }
    
    excluir(obj : any) : Observable<any>{
        return from(new Observable(observe => {

            this.firestore.collection('cliente').doc(obj.id).delete().then(response=>{
                observe.next("Excluído com sucesso");
            }, (err) => {
                observe.error("Erro ao cadastrar");
            });

        }));
    }

}


